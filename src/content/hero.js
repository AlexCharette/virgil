 ...
 * The corner indicators (watcher/snare sigils, eyes, charms) live in the
 * hero-badges module; the setWatchers/setSnares/setCharms/setWatcherFx methods
 * here just delegate to it.
 */
(function (g) {
  ...
  H.mount = function () {
    ...
    // The corner indicators + eyes live in their own module.
    V.heroBadges.mount(root, host);
  ...
  // The indicators (delegated to hero-badges).
  H.setWatcherFx = (on) => V.heroBadges.setWatcherFx(on);
  H.setWatchers = (n) => V.heroBadges.setWatchers(n);
  H.setSnares = (n, onClick) => V.heroBadges.setSnares(n, onClick);
  H.setCharms = (ids) => V.heroBadges.setCharms(ids);
